//在自己所建的buffer中設計，若要put就要用boolean值裡面是空的，否則就wait。若要get就要確定裡面是有東西的，否則wait。
package Fig23_16_17;
...
public class SynchronizedBuffer implements Buffer
{
   ...
   private boolean occupied = false; //裡面是否有東西
      ...
      throws InterruptedException
   {
      ...
      while (occupied) 
      {
         ...
         wait(); //把呼叫put的Thread放進SynchronizedBuffer的wait queue
      }
      ...
      notifyAll(); // 把此物件中所有的Thread都叫醒 // tell waiting thread(s) to enter runnable state //下保險，不一定會用到。除非因為執行時間太久被丟回Ready Queue
